...
public class ImageUtil {
    ...
    public static String getPhotoSavePath(Context context) {
        ...
        if (photoFolder == null) {
            photoFolder = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/xuanbao/files/photos");
        }
        String id = UUID.randomUUID().toString();//生成的id942cd30b-16c8-449e-8dc5-028f38495bb5中间含有横杠
        id = id.replace("-", "");//替换掉中间的那个斜杠
        return new File(photoFolder, "and_" + id + ".jpg").getAbsolutePath();
    }
     ...
     * @return
     */
    public static String compressImg(Context context, String path, float scale, int quality) {
        String outputPath = getPhotoSavePath(context);
        ...
        //图片大小比预设的最大size大,需要压缩
        File outputFile = new File(outputPath);
        Log.d("ImageUtil", "inSampleSize 压缩后 宽度为" + bmp.getWidth() + "高度为" + bmp.getHeight());
        ...
        Log.d("ImageUtil", "压缩后大小：" + outputFile.length());
        return outputPath;
    }
     ...
     * @return
     */
    public static String compressImg(Context context, Bitmap bm, int quality) {
        String outputPath = getPhotoSavePath(context);
        BitmapFactory.Options opts = new BitmapFactory.Options();
        ...
        //图片大小比预设的最大size大,需要压缩
        File outputFile = new File(outputPath);
        Log.d("ImageUtil", "inSampleSize 压缩后 宽度为" + bm.getWidth() + "高度为" + bm.getHeight());
        FileOutputStream fos = null;
        try {
            fos = new FileOutputStream(outputFile);
            //质量压缩
...
//            copyFileUsingFileChannels(tempFile, outputFile);
     ...
     * @return
     */
    public static String compressImg(Context context, Bitmap bm, int quality, float outW, float outH) {
        String outputPath = getPhotoSavePath(context);
        BitmapFactory.Options opts = new BitmapFactory.Options();
        ...
        int zoomRatio = 1;
        if (w > h && w > outW) {
            zoomRatio = (int) (w / outW);
        } else if (w < h && h > outH) {
            zoomRatio = (int) (h / outH);
        }
        if (zoomRatio <= 0)
            zoomRatio = 1;
        opts.inSampleSize = zoomRatio;
...
//                bitmap.recycle();
    ...
    public static TransitionDrawable bitmapToTransitionDrawable(Bitmap bitmap) {
        ...
        try {
            ...
            mBitmapDrawable = new TransitionDrawable(new Drawable[]{new ColorDrawable(0x000000), new BitmapDrawable(bitmap)});
        } catch (Exception e) {
            e.printStackTrace();
        }
        return mBitmapDrawable;
    ...
    public static TransitionDrawable drawableToTransitionDrawable(Drawable drawable) {
        ...
        try {
            ...
            mBitmapDrawable = new TransitionDrawable(new Drawable[]{new ColorDrawable(0x000000), drawable});
        } catch (Exception e) {
            e.printStackTrace();
        }
        return mBitmapDrawable;
     ...
     *
     * @author gengqiquan
     * @date 2017/6/30 上午9:43
     */
    public static boolean ImageEquals(Bitmap bmpOne, Bitmap bmpTwo) {

        for (int i = 0; i < bmpOne.getWidth(); i++) {
            for (int j = 0; j < bmpOne.getHeight(); j++) {
